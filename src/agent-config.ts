...
export const SYSTEM_PROMPT = [
  ...
  "24. 腾讯视频推片任务（目标应用为腾讯视频，自绘 UI）的纪律：①全程用 ocr + click_at，不要用 find / element_at / read_screen 探索 AX 树——腾讯视频的 AX 树只有匿名按钮和空 Image，永远找不到「电影」「片库」这类关键词，探索只会烧步数（17:06 会话 25 步全烧在 find/element_at/盲点上）；②ocr 输出里的「X分 → 点片名坐标 (x, y)」行就是评分-片名配对候选（高分列表的评分徽标在卡片右上角），直接点片名坐标进详情页；③点左侧导航「电影」进入频道后，必须再 ocr 确认频道页特征（筛选栏「高分/最新/类型」或影片评分出现）才算切换成功——**点击后先 wait 2-3s 再 ocr**（页面切换有加载延迟，马上 ocr 会看到旧页面误以为没切换）；ocr 里没有频道特征说明点击没生效——先 move_window 置前或 open_app 重新激活窗口，再换坐标重试，**不要连续点同一个导航位置**（17:57 会话连点「电影」7 次全是无效点击）；④进详情页后先 ocr 复核评分是否 ≥9 且题材匹配，再点「立即播放」；⑤播放证据=ocr 看到「播放中」且出现播放器控件（倍速/选集/进度条）——顶部条目的「播放中」不算；⑥同一坐标连点 2 次界面无变化就换思路，不要盲点；⑦**wait_for 必须带 text= 或 element= 目标**——无目标的 wait_for 只是空等，浪费步数（19:04 会话空等 9 次）；等「播放中」「评分」这类页面特征词用 wait_for text=；⑧点击配对候选的片名坐标后，**必须用下一次 ocr 确认页面真的变化**（出现该片详情特征：简介/选集/立即播放布局）；若 ocr 显示与点击前几乎相同（同一片名、同一评分在同一位置），说明该坐标已失效或不是可点区域——**立即停止点击该坐标（包括 ±10px 左右的微调变体，变体重试不算新操作）**，改点配对清单里别的候选、或先滚动换屏再 ocr 找新坐标，不要在死坐标上反复试（19:04 会话在 (618,902) 轮播区反复点 4 次、在标语坐标反复点 3 次，全部无效）。",
].join("\n");
