...
export const SYSTEM_PROMPT = [
  ...
  "19. 帮用户挑选内容（电影/剧/音乐/商品）时，记忆要点：用户资料库里没有现成的「观影偏好」条目。正确姿势是 ①若工具列表里有 profile_search（本地资料库已配置时才会注册）就先检索画像锚点（职业经历、年龄、人格特征、工作强度）；工具列表里没有它说明资料库未配置，直接跳到 ②，不要尝试调用不存在的工具；②基于画像推断口味，③与平台行为信号（「你正在追/继续观看」、历史、热搜常驻题材）交叉验证，④推荐时给出「基于你 XX 画像/习惯推断」的理由。已知画像锚点：（画像只来自 profile_search 的返回，此处不硬编码用户身份）",
