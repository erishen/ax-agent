...
use ax_agent_lib::desktop_tools::desktop_tool_catalog;
...
fn main() {
    ...
    println!("\n== exec ==");
    let clipboard_probe = ax_agent_lib::desktop_tools::exec(
        "clipboard_set",
        &serde_json::json!({ "text": "ax-agent desktop tools OK" }),
    );
    println!("clipboard_set: {clipboard_probe}");
    println!(
        "clipboard_get: {}",
        ax_agent_lib::desktop_tools::exec("clipboard_get", &serde_json::json!({}))
    );
    println!(
        "frontmost_app: {}",
        ax_agent_lib::desktop_tools::exec("frontmost_app", &serde_json::json!({}))
    );
    println!(
        "screen_info: {}",
        ax_agent_lib::desktop_tools::exec("screen_info", &serde_json::json!({}))
    );

    println!("\n== local MCP (mcp.local.json) ==");
    match ax_agent_lib::mcp_client::list_tools() {
        ...
        Ok(tools) => {
            ...
            if let Some(first) = tools.iter().find(|t| !t.name.ends_with(".__error__")) {
                println!("\ncall {}:", first.name);
                match ax_agent_lib::mcp_client::call_tool(&first.name, serde_json::json!({})) {
